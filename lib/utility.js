...
function rigaSemplice(text, indent) {
  ...
  console.log(`${VERDE}:::${FINE} ${" ".repeat(indent)}${prima}`);
  for (const r of dopo) {
    console.log(`${VERDE}:::${FINE} ${" ".repeat(indent + HANG)}${r}`);
  }
}
...
function riga(etichetta, stile, text) {
    if (semplice) {
        ...
        if (etichetta) console.log(`${VERDE}:::${FINE} ${STILE[stile](etichetta)}`);
...
export function logRule(etichetta = "", stile = "normale") {
    if (semplice) {
        ...
        // che allinea.
        console.log(etichetta ? `${VERDE}:::${FINE} ${STILE[stile](etichetta)}` : `${VERDE}:::${FINE}`);
        return;
    }
    console.log(
 ...
 * @param {string} testo
 * @param {{ closingRule?: boolean }} [opts] - `closingRule: false` per chi vuole che il corpo
 *   del comando prosegua senza soluzione di continuità sotto l'intestazione, riservando le
 *   traverse a ciò che le merita davvero: separare un blocco di avvisi finale dal resto (vedi
 *   `--fastverify` e la sincronizzazione in cli.js). Ignorato in modalità semplice, che non ha
 *   comunque traverse.
 */
export function logHeader(nome, versione, testo, { closingRule = true } = {}) {
    if (semplice) {
        // Nome e versione stanno su una riga sola: non c'è più una traversa che apre il
        // blocco su cui appoggiare l'una e una colonna d'etichetta su cui appoggiare l'altra.
        console.log(`${VERDE}:::${FINE} ${STILE.normale(versione ? `${nome} ${versione}` : nome)}`);
        rigaSemplice(testo, 0);
        return;
    }
    logRule(nome);
