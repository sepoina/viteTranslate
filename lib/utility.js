...
/**
 * Taglia `s` a `max` colonne di terminale, con "…" dove ha tagliato. Conta come `displayWidth`
 * — le sequenze di colore zero colonne, un carattere largo due — e le sequenze restano al loro
 * posto: dopo il taglio si chiude con un reset, altrimenti un colore rimasto aperto a metà si
 * allungherebbe su tutto quello che il terminale scrive dopo.
 *
 * È il contrario di `wrapLog`: dove una riga di log va a capo, una riga che si riscrive sul
 * posto (vedi `logLineRows`) non può — ogni riga in più sposterebbe tutte quelle sotto.
 *
 * @param {string} s
 * @param {number} max
 * @returns {string}
...
export function clipToWidth(s, max) {
    ...
    if (displayWidth(testo) <= max) return testo;
    if (max < 1) return "";
    let out = "";
    let usate = 0;
    for (const pezzo of testo.split(/(\x1b\[[0-9;]*m)/)) {
        if (pezzo.startsWith("\x1b[")) { out += pezzo; continue; }
        for (const g of graphemes(pezzo)) {
            const w = displayWidth(g);
            if (usate + w > max - 1) return `${out}…${FINE}`;
            out += g;
            usate += w;
        }
    }
...
// Stato di modulo, non parametro su ogni chiamata di stampa: i punti di stampa sono una
// trentina sparsi in quattro file, e passarlo a mano vorrebbe dire che il primo dimenticato
// stampa nell'altra forma.
let semplice = false;

/** @param {{ simple: boolean }} opts */
export function setLogStyle({ simple }) {
  semplice = !!simple;
}

/** In simple mode sono sparite la colonna dell'etichetta e le traverse: vedi `riga`, `logRule`. */
export const isSimpleLog = () => semplice;

// Indentazioni del rendering semplice: solo tre valori in tutto l'output, invece della colonna
// a larghezza fissa del rendering ricco. Le continuazioni rientrano sempre di due in più della
// prima riga del loro blocco, stessa regola di HANG in rich mode.
const SIMPLE_TEXT_INDENT = 2;
const SIMPLE_BULLET_INDENT = 4;

// I pezzi di riga prima del testo. Una copia sola, per chi stampa (`riga`, `rigaSemplice`) e
// per chi restituisce le righe senza stamparle (`logLineRows`): due copie vorrebbero dire una
// riga riscritta sul posto che non cade nelle stesse colonne del log che poi la sostituisce.
const prefissoSemplice = (indent) => `${VERDE}:::${FINE} ${" ".repeat(indent)}`;
const etichettaSemplice = (etichetta, stile) => `${VERDE}:::${FINE} ${STILE[stile](etichetta)}`;
const prefissoRicco = (etichetta, stile) =>
    `${VERDE}:::${FINE}${TENUE} ${FINE}${STILE[stile](fit(etichetta, LABEL))}${TENUE} ║  ${FINE}`;

/** Una riga (o le sue continuazioni) nel rendering semplice, a partire da `indent`. */
function rigaSemplice(text, indent) {
  const [prima, ...dopo] = wrapLog(text);
  console.log(`${prefissoSemplice(indent)}${prima}`);
  for (const r of dopo) {
    console.log(`${prefissoSemplice(indent + HANG)}${r}`);
  }
}
...
function riga(etichetta, stile, text) {
    if (semplice) {
        // L'etichetta, se c'è, esce prima sulla sua riga: senza la colonna a larghezza fissa
        // non ha più un posto dove stare accanto al testo.
        if (etichetta) console.log(etichettaSemplice(etichetta, stile));
        rigaSemplice(text, SIMPLE_TEXT_INDENT);
        return;
    }
    const [prima, ...dopo] = wrapLog(text);
    console.log(`${prefissoRicco(etichetta, stile)}${prima}`);
    for (const r of dopo) {
...
/**
 * Quante colonne restano al testo di una riga di log in un terminale largo `larghezza`: quelle
 * della colonna del log (`TEXT_WIDTH`), o meno se il terminale è più stretto del log.
 *
 * @param {number} [larghezza]
 * @returns {number}
 */
export function logTextWidth(larghezza = LOG_WIDTH) {
    const margine = semplice ? displayWidth(prefissoSemplice(SIMPLE_TEXT_INDENT)) : GUTTER;
    return Math.max(0, Math.min(TEXT_WIDTH, larghezza - margine));
}

/**
 * Le righe di terminale di `logEchoColored(msg, text, stile)`, restituite invece che stampate,
 * e senza andare a capo: il testo che non ci sta si tronca (`clipToWidth`). Stessa colonna,
 * stessa etichetta, stesso `--simpleLog` di una riga stampata — gli stessi prefissi, non una
 * copia — così la regione che si riscrive sul posto (lib/dev/llm/liveRegion.js) e il log che
 * alla fine prende il suo posto cadono nelle stesse colonne.
 *
 * @param {string} msg - l'etichetta
 * @param {string} text - una riga sola, senza "\n"
 * @param {"normale"|"nome"|"ok"|"warning"|"error"} [stile]
 * @param {number} [larghezza] - le colonne a disposizione: nessuna riga restituita le supera
 * @returns {string[]} una riga; due in simple mode con un'etichetta, che lì sta sulla sua
 */
export function logLineRows(msg, text, stile = "normale", larghezza = LOG_WIDTH) {
    const corpo = clipToWidth(text, logTextWidth(larghezza));
    const righe = semplice
        ? [...(msg ? [etichettaSemplice(msg, stile)] : []), `${prefissoSemplice(SIMPLE_TEXT_INDENT)}${corpo}`]
        : [`${prefissoRicco(msg, stile)}${corpo}`];
    return righe.map((r) => clipToWidth(r, larghezza));
}
...
export function logRule(etichetta = "", stile = "normale") {
    if (semplice) {
        // Senza etichetta è la riga vuota di stacco; con etichetta si legge come un'etichetta
        // qualsiasi — la traversa stessa (il filetto `─`) non ha più senso senza la colonna
        // che allinea.
        console.log(etichetta ? etichettaSemplice(etichetta, stile) : `${VERDE}:::${FINE}`);
        return;
    }
    console.log(
...
/**
 * La riga di dettaglio di un blocco di avviso (un file fra tanti, un percorso saltato). In
 * rich mode il rientro del puntato era una decisione presa dentro le stringhe dei chiamanti
 * (`"- " + msg`); qui è la funzione a deciderlo, cosa che in simple mode serve per dargli un
 * rientro proprio invece del rientro del testo normale.
 *
 * @param {string} text - senza il trattino iniziale: lo aggiunge questa funzione
 */
export function logBullet(text) {
    if (semplice) {
        rigaSemplice(`- ${text}`, SIMPLE_BULLET_INDENT);
        return;
    }
    logEchoColored("", `- ${text}`);
}
