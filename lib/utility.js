...
/**
 * Quanto è lungo il filetto della traversa. Corto di proposito: serve a separare i blocchi,
 * non a incorniciare il testo — tirato fino ai 120 diventerebbe la cosa più vistosa
 * dell'output, e a doversi vedere sono gli avvisi.
 */
const RULE_WIDTH = 76;
...
export function logHeader(nome, versione, testo) {
    logRule(nome);
    // Incolonnata a destra sotto il nome, con un margine prima del montante: appesa al bordo
    // sembrerebbe parte della cornice invece che un dato.
    const margine = 3;
    const etichetta = "⌘ "+versione;
    riga(etichetta, "normale", testo);
    logRule();
}
