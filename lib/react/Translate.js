...
export default function Translate({ t = false, a = false, o = false, children = false, skipMark = false }) {
  ...
  if (isCompiledMarker(text)) {
    ...
    return resolveEntry(lang?.table, fallbackTable, markerKey(text), args, text, diag, lang?.icu ?? manifest.icu);
  }
  //
...
function salvage(props, lang, key, build) {
  ...
  const node = isCompiledMarker(text)
    ? resolveEntry(lang?.table, fallbackTable, markerKey(text), args, text, quiet, lang?.icu ?? manifest.icu)
