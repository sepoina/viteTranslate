...
export default function Translate({ t = false, a = false, o = false, children = false }) {
  ...
  if (
    ...
    reportOnce(diag, `Translate: object without a "t" field is not a { t, a } form and renders empty: ${describeValue(source)}`);
    return "";
  }
  ...
  if (Array.isArray(source)) {
    if (a !== false) return salvage({ t, a, o, children }, lang, `Translate: "a" cannot be set when using the t:${describeValue(t)} array form`);
    [text, ...args] = source;
  }
  //
  // formato classico t="..." a=[arg1, arg2, ...]
    ...
    if (typeof source === "object" && !(source instanceof String)) {
      return salvage({ t, a, o, children }, lang, `Translate: "t" must be a string, an array or a { t, a } object, got ${describeValue(source)}`);
    }
    text = source;
    args = a ?? [];
  }
