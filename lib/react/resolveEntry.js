...
// Chiave assente da entrambe le tabelle. Con la lingua sorgente sempre importata staticamente
// dal plugin, e garantita completa dal comando di sincronizzazione prima della build, qui non
// si dovrebbe mai arrivare: se succede è un file lingua non sincronizzato, e va detto forte
// invece di essere mascherato. Il fallback incorporato nel marcatore è testo grezzo, non
// compilato, quindi il markup che contenesse resterebbe visibile: si interpola soltanto, così
// almeno i `%s` non finiscono a schermo.
function missing(key, args, fallback) {
  if (import.meta.env?.DEV) {
    console.error(`Translate: chiave "${key}" assente dalla tabella e dalla lingua sorgente — file lingua non sincronizzato?`);
  }
