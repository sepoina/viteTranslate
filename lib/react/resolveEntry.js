...
import { UNTRANSLATED_KEY } from "../markerSyntax.js";
 ...
 * @param {{timeZone?: string}} [icu] - opzioni di runtime per i messaggi ICU (piano 4.6.3): per
 *   ora solo il fuso orario, risolto da `TranslateContainer` o dal default del plugin
 *   (`lang?.icu ?? manifest.icu`) nei punti di chiamata.
 * @returns {React.ReactNode}
 */
export function resolveEntry(table, fallbackTable, key, args, marker, diag = DEFAULT_DIAGNOSTICS, icu) {
  ...
  if (entry === undefined || entry === null) return withPrefix(prefix, missing(key, args, marker, diag, icu));
 ...
 * @returns {string}
 */
export function resolveEntryText(table, fallbackTable, key, args, marker, diag = DEFAULT_DIAGNOSTICS, icu) {
  const resolved = resolveEntry(table, fallbackTable, key, args, marker, diag, icu);
...
function missing(key, args, marker, diag, icu) {
  ...
  if (import.meta.env?.DEV && fallback !== undefined) {
    // Chiave scritta in questa sessione di dev, non ancora sincronizzata: se il testo di
    // riserva è un messaggio ICU, l'interprete di dev lo riduce a "%s" + valori prima di
    // passarlo al parser HTML di sempre — altrimenti si vedrebbe la sintassi grezza.
