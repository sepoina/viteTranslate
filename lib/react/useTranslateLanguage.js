...
import * as virtualModule from "virtual:vitetranslate/languages";
import { resolveDiagnostics, report } from "../errorSolve.js";

const diag = resolveDiagnostics(virtualModule);

// Elenco { tag, languageName } trovato in localeDir, lingua sorgente per prima: si ricava
// dal solo manifest delle lingue (nomi già calcolati alla generazione del manifest, vedi
// vitetranslate.js), senza caricare nessuna tabella. È una costante di modulo, quindi è
// disponibile anche fuori da <TranslateContainer> e non cambia mai identità.
//
...
function proposeNewLanguageOutsideContainer() {
  ...
  report(diag, "error", "useTranslateLanguage: proposeNewLanguage() called outside <TranslateContainer>, the language switch has no effect");
}
