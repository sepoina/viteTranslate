...
export function useTranslateToString() {
  ...
  return useCallback(function ts(t, a, options) {
    ...
    // Confronto esplicito e non sulla falsità del valore, come in <Translate>: `0` è un testo
    // legittimo, `false` è la sentinella del "non passato".
    ...
    // restituire una stringa primitiva, e un nodo montato a stringa non si riduce. Cadeva già
    // nel controllo qui sotto, ma con il messaggio generico dell'oggetto senza `t`, che di
    // questo caso non dice niente.
    ...
    // Un oggetto senza campo `t` è "niente", come in <Translate>: una variante di null,
    // non testo da mostrare. Prima arrivava a "[object Object]" con il prefisso `‼️`.
    // L'uso è comunque scorretto: si segnala in console, se i flag lo consentono.
    if (typeof source === "object" && !Array.isArray(source) && !(source instanceof String) && !Object.hasOwn(source, "t")) {
      // Chiave statica, messaggio pigro: vedi la nota gemella in Translate.js.
      reportOnce(
        diag,
        'useTranslateToString: object without a "t" field',
        () => `useTranslateToString: object without a "t" field is not a { t, a } form and renders empty: ${describeValue(source)}`,
      );
      return '';
    }
    ...
    // Un numero nella posizione del testo è dato di dominio, come in <Translate>: marcato non
    // può essere, e non deve portarsi dietro né il prefisso né il warning del non marcato.
