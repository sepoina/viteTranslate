// Architettura d'insieme: doc/structure.md § "Fase 4 — Runtime" e "Il dialetto HTML, in un posto solo".
...
// Dialetto condiviso con il parser di build (lib/dev/compile/parseMarkup.js): una sola
// definizione, così i due lati non possono divergere.
import { ALLOWED_TAGS, VOID_TAGS, HAS_HTML_RE } from "../htmlDialect.js";
import { DEFAULT_DIAGNOSTICS, report } from "../errorSolve.js";
// --- HELPERS ---

// Cache degli alberi già prodotti. La funzione è pura e gli elementi React sono immutabili,
// quindi lo stesso albero si riusa fra chiamate: il parsing di una stringa si paga una volta
// sola. La chiave è la stringa già interpolata, che codifica in un solo valore testo e
// argomenti. Il limite FIFO c'è perché gli argomenti possono generare stringhe illimitate
// (contatori, timer, input dell'utente): circa 1 kB a voce, quindi il tetto vale circa 256 kB.
//
// Nota sul dimensionamento: da quando le tabelle di lingua sono compilate a build time,
// <Translate> non passa più di qui e questa cache serve due soli casi, entrambi marginali —
// il fallback di sviluppo in resolveEntry e le chiamate dirette di chi usa la funzione come
// API pubblica. Il tetto è quindi molto più largo del necessario, non un budget calibrato.
const CACHE = new Map();
const CACHE_MAX = 256;
let template = null;
...
function parseHtml(html) {
  ...
  // createElement e non un frammento JSX: è l'unico punto del file che usava sintassi JSX, e
  // costringeva l'estensione a .jsx. Senza, il modulo è caricabile anche da Node puro — serve
  // a resolveEntry, che lo importa per il fallback di sviluppo. Il resto del file già costruiva
  // gli elementi così.
 ...
 *   `interpolate`).
 * @param {{absentDataInArray: string, warn: boolean}} [diag] - configurazione diagnostica risolta, passata
 *   da chi la conosce (`resolveEntry`). Chiamandola come API pubblica valgono i default, che
 *   sono il comportamento di sempre.
 ...
 */
export function basicHtmlToNodes(text, args, diag = DEFAULT_DIAGNOSTICS) {
  const html = interpolate(text, args, diag);

  if (!HAS_HTML_RE.test(html)) return html; // text node puro: mai in cache

  // Senza DOM non c'è nulla da parsare. La guardia è esplicita e non un ReferenceError
  // lasciato cadere nel catch qui sotto: quella strada funzionava, ma stampava un errore
  // in console a ogni chiamata (il risultato non entra in cache), e in SSR le chiamate
  // sono una per stringa per richiesta.
  ...
  try {
    ...
    report(diag, "error", "basicHtmlToNodes: HTML parsing error", e);
    return html;
  }
