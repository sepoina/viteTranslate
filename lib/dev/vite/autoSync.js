...
async function esegui({ config, env, probeBabel, inBuild }) {
  try {
    // G10. Senza @babel/core la scansione non estrae niente: sourceTable resta vuota,
    // guardMassErase scatta con "nessun marcatore trovato" e mette in salvo una copia di ogni
    // lingua. Questa uscita anticipata è ciò che impedisce quel salvataggio inutile.
    //
    // Non è più l'ultima parola sul problema: l'auto-sync gira nel hook `config`, e subito
    // dopo `configureServer` o `buildStart` fermano tutto (vedi motivoBabel in
    // vitetranslate.js). Qui resta un avviso perché dice una cosa che quelli non dicono, cioè
    // che le tabelle non sono state toccate.
    try {
      probeBabel();
    } catch (errore) {
      if (errore?.code !== "VT_NO_BABEL") throw errore;
      logWarning(
        `auto-sync skipped: ${babelUnaRiga(errore.guasto)} ` +
        `Your tables were left untouched; run \`npx ${CLI_NAME}\` once that is fixed.`
      );
