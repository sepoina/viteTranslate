...
export default function guardMassErase(service, skippedCount) {
  ...
  let previous;
  try {
    ({ table: previous } = readLanguageFile(sourcePath));
  } catch {
  ...
  for (const file of languageFiles(localeDir)) {
    ...
    const saved = backupLanguageFile(filePath, file, text, { kind: "erased", reason: cause, detail: true });
...
function languageFiles(localeDir) {
  try {
    return listLanguageFiles(localeDir);
  } catch {
    return [];
  }
}
