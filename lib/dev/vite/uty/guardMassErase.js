...
import { languageFileName, isLanguageFileName } from "./languageFileFormat.js";
 ...
 * @returns {{ erased: string[], backups: string[] } | null} null se non c'è nulla
 *   di sospetto; altrimenti le chiavi in via di cancellazione e i backup effettuati
 */
export default function guardMassErase(service, skippedCount) {
  const { localeDir, sourceLanguage, sourceTable } = service;
  const sourcePath = pathCmd.join(localeDir, languageFileName(sourceLanguage));
  ...
  let previous;
  try {
    previous = readLanguageFile(sourcePath);
  } catch {
...
function languageFiles(localeDir) {
  try {
    return fs.readdirSync(localeDir).filter(isLanguageFileName);
  } catch {
    return [];
  }
}
