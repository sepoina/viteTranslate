...
import { LANGUAGE_KEY_RE } from "./languageFileFormat.js";
...
export default function serializeLanguageFile({ tag, isSource, translated, untranslated, now }) {
  ...
  const entryLine = ([key, value]) => {
    if (!LANGUAGE_KEY_RE.test(key)) {
      throw new Error(`[vitetranslate] key "${key}" cannot be written to a language file: it must match ${LANGUAGE_KEY_RE}`);
    }
