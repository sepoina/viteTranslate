...
export default function backupLanguageFile(filePath, fileName, originalText, { kind, reason, detail = false }) {
  ...
  try {
    ...
    avvisa(`'${colorize("nome", fileName)}' ${kind} (${reason}). Backup saved as '${colorize("nome", shortName)}', ${tail}`);
    return backupPath;
  } catch (backupError) {
    avvisa(`'${colorize("nome", fileName)}' ${kind} (${reason}) and backup failed (${backupError.message}); ${tail}`);
    return null;
  }
}
