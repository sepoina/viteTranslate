 ...
 *                                   e solo un oggetto: è l'unica voce che il resto della
 *                                   libreria dereferenzia senza chiedere permesso
 *
...
export default function parseLanguageFile(text, filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // permesso (`sourceTable.__builder__.v` quando genera una lingua nuova, e altrove):
    // cancellarne il valore a mano la lasciava passare come `null`, e il file si leggeva
    // benissimo per poi far cadere la build molto più tardi, su un TypeError senza numero di
    // riga. Qui è ancora una riga di un file, e si può dire quale.
    if (key === BUILDER_KEY && raw.charCodeAt(0) !== BRACE) {
      throw at(`"${BUILDER_KEY}" must hold a JSON object ({"v":…}) — it is generated data, run the sync to rebuild it`);
    }
