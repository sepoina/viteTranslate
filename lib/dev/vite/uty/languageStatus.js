...
import { displayWidth as width, wrapLog, logEchoColored, logWarning, logError, logRule, logBullet, colorize, TEXT_WIDTH } from "../../../utility.js";

// Stesso letterale che usa cli.js (vedi lì CLI_NAME): serve al comando da suggerire quando ci
// sono marcatori malformati non elencati per esteso (vedi printWarnings).
...
export function printWarnings({ warnings = [], skipped = [], dettaglio = false } = {}) {
  ...
  if (!dettaglio && malformati.length) {
    logWarning(`${malformati.length} malformed marker(s) in the source code, to see which files:`, { stacco: false });
    // Comando su una riga a sé — non in coda alla frase — per lo stesso motivo di
    // --fastverify: incollato in mezzo al testo, il wrap lo spezza a metà (era esattamente
    // il difetto di prima: "...to see which" andava a capo separato da "files"). "$ " davanti,
    // attenuato, e il comando per intero in verde: la stessa forma ovunque nel comando si
    // suggerisca qualcosa da digitare in un terminale (vedi anche cli.js, vitetranslate.js).
