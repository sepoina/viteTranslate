...
import { displayWidth as width, wrapLog, logEchoColored, logWarning, logError, logRule, logBullet, logCommand, colorize, TEXT_WIDTH } from "../../../utility.js";
import { CLI_NAME } from "./cliName.js";
import { parseIcu } from "../../../icu/parse.js";
import { compareIcu } from "../../compile/icu/icuSignature.js";

// Al massimo tre chiavi per riga, poi "…": le note sono già la parte che si allunga di più
// (piano 4.6.3).
const elencoChiavi = (keys) => (keys.length > 3 ? `${keys.slice(0, 3).join(", ")}, …` : keys.join(", "));
...
export function collectStatus(service, builderVersion) {
  ...
  const sourceKeys = Object.keys(sourceTable);
  ...
  for (const file of entries) {
    ...
    rows.push(statusOfFile(path.join(localeDir, file), file, { sourceLanguage, sourceKeys, builderVersion, sourceTable }));
  }
...
function statusOfFile(filePath, file, { sourceLanguage, sourceKeys, builderVersion, sourceTable }) {
  ...
  let table, meta;
  try {
    ({ table, meta } = readLanguageFile(filePath));
  } catch (e) {
  ...
  const chiavi = Object.keys(table);
  ...
  // Messaggi ICU (piano 4.6.3): sulla riga sorgente, sintassi ICU rotta nel proprio testo e
  // categorie plurali che mancano per la propria lingua; sulle altre righe, argomenti o
  // sintassi ICU che non combaciano col sorgente (compareIcu è l'unico confronto, lo stesso
  // della compilazione e del validatore LLM).
  const icuErrKeys = [];
  const icuWarnKeys = [];
  if (row.isSource) {
    for (const k of chiavi) {
      ...
      const parsed = parseIcu(v, tag);
      if (parsed.icu && !parsed.ok) icuErrKeys.push(k);
      if (compareIcu(v, v, tag).warnings.length > 0) icuWarnKeys.push(k);
    }
  } else if (sourceTable) {
    for (const k of chiavi) {
      const v = table[k];
      const sv = sourceTable[k];
      if (typeof v !== "string" || typeof sv !== "string") continue;
      const { errors, warnings } = compareIcu(sv, v, tag);
      if (errors.length > 0) icuErrKeys.push(k);
      if (warnings.length > 0) icuWarnKeys.push(k);
    }
  }
  if (icuErrKeys.length > 0) {
    nota("warning", row.isSource
      ? `${icuErrKeys.length} string(s) with invalid ICU, shown as plain text: ${elencoChiavi(icuErrKeys)}`
      : `${icuErrKeys.length} key(s) whose ICU arguments or syntax don't match the source, shown in ${sourceLanguage} until fixed: ${elencoChiavi(icuErrKeys)}`);
  }
  if (icuWarnKeys.length > 0) {
    nota("warning", `${icuWarnKeys.length} key(s) with incomplete plural or select branches: ${elencoChiavi(icuWarnKeys)}`);
  }
...
export function printWarnings({ warnings = [], skipped = [], dettaglio = false } = {}) {
  ...
  // seppellirebbero il riepilogo. `--status` le elenca comunque tutte. Annidati e collisioni
  // restano per esteso in entrambi i casi: sono rari, e ognuno riguarda un punto diverso del
  // codice.
  ...
  if (elencati.length) {
    ...
    for (const w of elencati) logBullet(w.message);
    stampato = true;
  }
  if (!dettaglio && compressi.length) {
    logWarning(`${compressi.length} marker warning(s) needing a file list, to see which files:`, { stacco: false });
  ...
  if (skipped.length) {
    ...
    for (const riga of skipped) logBullet(riga);
    stampato = true;
  }
  return stampato;
}
...
export function printStatus(status, info) {
  ...
  if (localeDirError) {
    ...
    logError(`the locale dir ${colorize("nome", `"${localeDir}"`)} cannot be read (${String(localeDirError).split(":")[0]})`, { stacco: false });
  ...
  const coda = [];
  if (sourceMissing) {
    coda.push(() => logError(`the source language ${colorize("nome", `"${sourceLanguage}"`)} has no file here: the plugin will not start`, { stacco: false }));
  }
  if (legacy.length) {
    coda.push(() => logError(`${legacy.length} file(s) still in the 3.x format (${legacy.map((f) => colorize("nome", f)).join(", ")}): run --migrate`, { stacco: false }));
  }
