...
import writeLanguageFileIfChanged from "./writeLanguageFile.js";
...
export default function migrateLegacyLanguages(localeDir, sourceLanguage) {
  ...
  if (files.length === 0) {
    logEchoColored("migrate", `No "${LEGACY_LANG_EXT}" language file found in ${colorize("nome", `"${localeDir}"`)}: nothing to migrate.`);
  ...
  for (const file of files) {
    ...
    if (fs.existsSync(targetPath)) {
      skipped.push([file, `${colorize("nome", `"${languageFileName(tag)}"`)} already exists`]);
      continue;
    }
  ...
  for (const [file, reason] of skipped) {
    logWarning(`${colorize("nome", `"${file}"`)} not converted (${reason}): left where it is.`);
  }
