...
import { logRule, logEchoColored, logCommand, colorize } from "../../../utility.js";
 ...
 * @param {object|null} [llm] - la config `llm`, se c'è: è la condizione perché il suggerimento
 *   `npx … --llm-translate` compaia.
 * @param {{ showTranslateHint?: boolean }} [opts] - `false` quando la sync gira dentro un run
 *   LLM: la riga "still untranslated" resta, il comando no.
 */
export function printSyncSummary({ action, written, languages }, sourceLanguage, llm = null, { showTranslateHint = true } = {}) {
  ...
  // `icuMismatch`: traduzioni ICU che il compilatore scarta (vedi updateAllSubLanguages.js).
  // `?? 0` per chi costruisce le voci a mano senza il campo.
  const icu = (l) => l.icuMismatch ?? 0;
  const complete = languages.filter((l) => l.note === null && l.missing === 0 && icu(l) === 0);
  if (complete.length) {
  ...
  for (const l of languages.filter((x) => x.note !== null || x.missing > 0 || icu(x) > 0)) {
    const parti = [];
    if (l.missing > 0) parti.push(`${l.missing} key(s) missing`);
    if (icu(l) > 0) parti.push(`${icu(l)} ICU key(s) not matching the source (see --status)`);
    const stato = parti.length > 0 ? parti.join(", ") : "complete";
  ...
  // Solo quando `llm` è configurato e resta davvero qualcosa da tradurre: altrimenti il
  // suggerimento comparirebbe anche a chi non ha mai acceso la funzione, o quando non
  // servirebbe a niente lanciarlo.
  const stillMissing = languages.reduce((sum, l) => sum + l.missing, 0);
  if (llm && stillMissing > 0) {
    logEchoColored("", `${stillMissing} string(s) still untranslated`);
    if (showTranslateHint) logCommand(`npx ${CLI_NAME} --llm-translate`);
  }
}
