...
import maybeRunLlmCommand, { hasAnyLlmFlag } from "../llm/llmCommands.js";
...
Examples:
  ...
  npx ${CLI_NAME} --status
  npx ${CLI_NAME} --llm-translate fr-FR --llm-dry-run
...
async function main() {
  ...
  if (fastVerifyFlag && (collectAddTags(process.argv) !== null || process.argv.includes("--status") || process.argv.includes("--migrate") || hasAnyLlmFlag(process.argv))) {
    throw new Error(
      "--fastverify cannot be combined with --add, --status, --migrate or any llm flag: those commands are asked for on purpose, and 'maybe nothing' is not one of their outcomes."
    );
  }
  ...
  if (fastVerifyFlag) {
    ...
    if (esito.fresh) {
      ...
      let languageCount = 0;
      try { languageCount = listLanguageFiles(path.join(process.cwd(), esito.localeDir)).length; } catch { /* non esiste ancora */ }
      printHeader({ srcLabel: esito.srcDir, fileCount: esito.checked, keyCount: esito.keys, localeLabel: esito.localeDir, languageCount });
  ...
  if (await maybeRunLlmCommand(process.argv, config)) return;
