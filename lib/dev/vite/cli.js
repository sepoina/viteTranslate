...
import languageAutonym, { shortAutonym } from "./uty/languageAutonym.js";
...
import packageVersion from "./uty/packageVersion.js";
import { languageFileName } from "./uty/languageFileFormat.js";
import { logEchoColored, logHeader, logRule, colorize, setLogStyle } from "../../utility.js";
...
import walkSource from "./uty/walkSource.js";
import { CONFIG_FILES, findConfigFile } from "./uty/configFiles.js";
...
async function loadExtractMarkers() {
  ...
  try {
    ...
    throw new Error(
      `scanning the source needs "@babel/core", which is not installed.\n` +
      `  ${consiglioPacchetto("@babel/core")}`
    );
  }
 ...
 */
async function scanSource(service, srcRoot) {
  let files;
  try {
  ...
  if (Object.keys(service.sourceTable).length - 1 === 0) {
    avviso(
 ...
 * nominarle insieme toglie di mezzo l'equivoco fra "dove cerco i marcatori" e "dove stanno le
 * tabelle" — che sta lì sotto, e che la scansione salta.
 *
 * Il conteggio fra parentesi dopo `translations:` sono le chiavi trovate NEL CODICE, cioè
 * quante dovrebbero essercene in ogni tabella: non quante ne contiene un file. Quando le due
 * cose non coincidono lo dice la colonna KEYS di `--status`, riga per riga.
 *
 ...
 *
 * Senza la traversa di chiusura (`closingRule: false`): il corpo del comando prosegue sotto
 * senza interruzioni, e la traversa si vede una sola volta prima degli avvisi finali — vedi
 * il commento sopra `printWarnings` più sotto, sullo stesso principio già usato da `printStatus`.
 *
 * @param {{ srcLabel: string, fileCount: number, keyCount: number, localeLabel: string }} p
 */
function printHeader({ srcLabel, fileCount, keyCount, localeLabel }) {
  const quanti = (n, cosa) => `${n} ${cosa}${n === 1 ? "" : "s"}`;
  logHeader("viteTranslate", packageVersion(),
    `sources: ${colorize("nome", `"${srcLabel}"`)} (${quanti(fileCount, "file")}, ${quanti(keyCount, "sentence")}),  ` +
    `translations: ${colorize("nome", `"${localeLabel}"`)} `,
    { closingRule: false });
}
 ...
 */
function testoMotivo({ reason, detail, others }) {
 ...
 */
function printSyncSummary({ action, written, languages }, sourceLanguage) {
...
async function main() {
  ...
  if (fastVerifyFlag) {
    ...
    if (esito.fresh) {
      ...
      if (esito.warnings > 0) {
        ...
        // Riga a sé, non in coda alla precedente con un "\n": un "\n" dentro lo stesso
        // logEchoColored la stamperebbe come continuazione, rientrata di due in più (vedi
        // HANG in utility.js) — qui invece deve restare alla stessa colonna delle altre righe.
        // "$ " davanti, come un prompt di shell: è la convenzione più universale per dire
        // "questo si digita in un terminale", la stessa di ogni man page e ogni README (vedi
        // anche fixCommand in vitetranslate.js, stessa forma). Attenuato ("normale" = dim),
        // perché il prompt non è l'informazione: è il comando che deve saltare all'occhio.
        logEchoColored("", `${colorize("normale", "$")} ${colorize("ok", `npx ${CLI_NAME} --status`)}`);
      }
      logRule();
  ...
  const chiaviTrovate = Object.keys(service.sourceTable).length - 1; // meno __builder__
