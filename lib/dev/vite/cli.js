...
import { logEchoColored, logHeader, logRule, colorize } from "../../utility.js";
 ...
 * @returns {Promise<{ files: string[], skipped: string[], warnings: Array<{kind: string, message: string}> }>}
 */
async function scanSource(service, srcRoot) {
  ...
  if (Object.keys(service.sourceTable).length - 1 === 0) {
    avviso(
      `no marker found in "${shortPath(srcRoot)}" (${files.length} file(s) scanned): ` +
 ...
 */
function printHeader(service, srcRoot, files) {
  ...
  logHeader("viteTranslate", packageVersion(),
    `sources: "${shortPath(srcRoot)}" (${quanti(files.length, "file")}, ${quanti(chiavi, "sentence")}),  ` +
    `translations: "${shortPath(service.localeDir)}" `);
}
...
function printSyncSummary({ action, written, languages }, sourceLanguage) {
  ...
  for (const l of languages.filter((x) => x.note !== null || x.missing > 0)) {
    ...
    logEchoColored("", colorize("warning", `${nome(l.tag, true)} - ${l.note ? `${l.note}, ` : ""}${stato}`));
  }
}
...
async function main() {
  ...
  const { files, skipped, warnings } = await scanSource(service, srcRoot);

  printHeader(service, srcRoot, files);
  ...
  // Gli avvisi sul sorgente PRIMA di sincronizzare, non dopo: dicono che alcune chiavi sono
  // nate storte, e stanno per finire in tutte le tabelle che il riepilogo qui sotto riassume.
  // Letti dopo, sembrerebbero un commento a un lavoro già finito bene.
  //
  // Dopo un --add no: lì a chiudere c'è il rapporto completo, che li stampa già per esteso
  // (printStatus), e uscirebbero due volte a poche righe di distanza.
  if (daAggiungere === null && printWarnings({ warnings, skipped })) logRule();
