...
LLM auto-translation (needs an "llm" block in vite.config — see doc/llm.md):
  --translate [tag...]  Sync, then fill the null keys with an LLM. No tags: every
                        language with missing keys.
  --dry-run             With --translate/--context: print the cost estimate and
                        exit. Sends nothing.
  --context             Regenerate the context abstract now. --context --show
                        prints it and exits.
  --llm-status          Connection, key source, context age, today's counters.
                        --ping also makes one minimal call to check the model
                        answers.
  --retranslate <tag>...  Redo already-translated keys too. Backs up first.
  --force               Bypass the numeric budget caps (not the CI guard).
  --yes                 Non-interactive confirmation.
  --key set|clear|status  Manage the API key in the system keyring, if installed.
