 ...
 * @returns {Promise<Array<{ tag: string, missing: number, icuMismatch: number, note: string|null }>>} una voce per
 ...
 * @param {object} service - Stato condiviso della sessione di sincronizzazione (vedi cli.js).
 * @returns {Promise<{ tag: string, missing: number, icuMismatch: number, note: string|null } | null>} null se
...
async function updateSingleSubLanguage(sourceDir, file, referenceTable, service) {
    ...
    try {
        ...
        //
        // `icuMismatch`: traduzioni ICU con argomenti o sintassi diversi dal sorgente. Non
        // mancano, ma il compilatore le scarta e mostra la sorgente (invariante 21): il
        // riepilogo non può dire "all ok!" per una lingua che ne ha.
        let icuMismatch = 0;
        for (const key in referenceTable) {
            const value = existingJson[key];
        ...
        return { tag, missing: untranslated.length, icuMismatch, note: nota };
    } catch (err) {
