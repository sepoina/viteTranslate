...
async function updateSingleSubLanguage(sourceDir, file, referenceTable, service) {
    ...
    try {
        ...
        // Aggiorna subito la versione: non è una chiave di contenuto da tradurre, va esclusa
        ...
        // "to be translated" anche nel file della lingua sorgente.
        existingJson["__lngVersion__"] = referenceTable["__lngVersion__"];
        ...
        // "(incomplete)" in coda a __lngVersion__ quando restano chiavi null in questo file:
        // se l'utente traduce le chiavi a mano fuori da questo comando (sostituendo i null
        // col testo, senza toccare la riga separatrice), i dati letti da disco risultano già
        // completi sia "prima" che "dopo" agli occhi di questo giro, quindi il confronto sotto
        // non vedrebbe alcuna differenza e la sezione "to be translated" resterebbe bloccata
        // nel file anche a traduzione conclusa.
        const versionEntry = translated.find(([key]) => key === "__lngVersion__");
        versionEntry[1] += untranslated.length ? " (incomplete)" : "";
