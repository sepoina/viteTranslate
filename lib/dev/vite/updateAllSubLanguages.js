...
function updateSingleSubLanguage(sourceDir, file, referenceJson, service) {
    ...
    try {
        ...
        logEchoColored("", `Error reading ${file}: ${err.message}`);
        return;
    }
    // Parsa il JSON dal contenuto del file. Se è corrotto, non lasciarlo bloccato per
    // sempre (il vecchio comportamento si limitava a loggare e saltare il file a ogni
    // giro): si salva una copia di backup e si riparte da una tabella vuota, così il
    // file torna in uno stato valido e compilabile.
    let existingJson;
    try {
