 ...
 * @param {{timeZone: string}|null} p.icuOptions - il default di build del fuso orario dei
 *   messaggi ICU (piano 4.6.3), già normalizzato da uty/icuOptions.js
 * @param {string|null} p.icuDevPath - percorso di lib/icu/devInterpret.js nell'installazione
 *   del pacchetto, o `null` in un'installazione atipica (vedi ICU_DEV in vitetranslate.js)
 * @returns {{ generate: () => Promise<string> }}
 */
export default function creaManifest({ defs, localeDir, reporter, leggiStato, icuOptions, icuDevPath }) {
  ...
  async function generate() {
    ...
    return (
      ...
      // Sempre presenti, anche quando l'ICU è spenta: un export che manca, letto da un
      // namespace (`import * as manifest`), fa scattare MISSING_EXPORT in build. In produzione
      // l'interprete di dev (e con lui il parser) non viene mai importato.
  ...
  function partiallyTranslated(tableByTag, tags, errorSolve) {
    ...
    for (const key of Object.keys(sourceTable)) {
      for (const tag of others) {
        ...
        // Una traduzione ICU con argomenti diversi dal sorgente il compilatore la scarta e
        // mostra la sorgente con `🔸` (invariante 21): per `🔹` è non tradotta quanto un null.
        // compareIcu esce subito se nessuno dei due testi è ICU.
        if (typeof sourceTable[key] === "string" && typeof value === "string"
          && compareIcu(sourceTable[key], value, tag).errors.length > 0) { keys[key] = 1; break; }
      }
    }
    return keys;
  }
