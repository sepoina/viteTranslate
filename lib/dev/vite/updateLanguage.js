...
export default async function updateLanguage(service) {
  ...
  // Il testo grezzo del file letto, o `null` quando non ce n'è uno affidabile da confrontare:
  // qualunque guasto (illeggibile, corrotto, assente, vuoto) forza la riscrittura sotto.
  let oldText = null;
  //
  ...
  // Confronto sui byte, mascherando solo la riga del timestamp (vedi uty/buildLanguageHeader.js):
  // `oldText` è `null` in ogni ramo di guasto (illeggibile, corrotto, assente, vuoto), e forza
  // la riscrittura sotto.
  ...
  // condizione la scrittura partirebbe comunque (oldText è null, quindi `unchanged` è
