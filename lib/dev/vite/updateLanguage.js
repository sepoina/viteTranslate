 ...
 *   { localeDir, sourceLanguage, sourceTable, notTranslated, notTranslatedString, renamedKeys }
 * @returns {void}
 *
 * @description
...
export default function updateLanguage(service) {
  ...
  const fileName = `${sourceLanguage}.json`;
  ...
  try {
    ...
    if (!baseData["__lngVersion__"]) throw new Error("Incorrect structure of json file");
    ...
    if (e.code === "ENOENT") {
      ...
      // Il file esiste ma non è leggibile (JSON non valido o struttura inattesa): non va
    ...
    originalText = null; // forza la scrittura sotto: il contenuto letto era corrotto/assente
  }
  //
  //
  //
  ...
  updateAllSubLanguages(filePath, service.sourceTable, service);
  //
