...
export default function vitetranslate(defs) {
  ...
  async function generateLanguagesModule() {
    ...
    const dirEntries = fs.readdirSync(localeDir);
    const files = dirEntries.filter(isLanguageFileName);
    ...
    if (!tags.includes(defs.sourceLanguage)) {
      ...
      if (legacy.length) {
        throw new Error(
          ...
          `From 4.0 they are data files, not JS modules: run "vitetranslate-prepare-translation-table --migrate" once to convert them to "${LANG_EXT}" (the originals are kept as .bak-migrated-*).`
        );
      }
      throw new Error(
