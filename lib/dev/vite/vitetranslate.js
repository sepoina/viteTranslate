...
import normalizeLlmOptions from "../llm/llmOptions.js";
import normalizeIcuOptions from "./uty/icuOptions.js";
...
// non contando le cartelle: con i workspace (site/pages/*) node_modules/@sepoina/
// vitetranslate è un SYMLINK che punta alla radice del repo, non a "lib/", e un calcolo
// relativo a quel link escluderebbe l'intero repo (site/pages/*/src compresi)
...
export default function vitetranslate(defs) {
  ...
  // Stessa disciplina di localeDir/sourceLanguage: un blocco `llm` scritto male fa vedere un
  // refuso all'avvio del dev server, non alla prima chiamata a pagamento. Il plugin non fa
  // nient'altro con questo valore: non risolve la chiave, non tocca la rete — vedi
  // doc/structure.md § "Phase 5 — LLM auto-translation", invariante 18.
  const llm = normalizeLlmOptions(defs);
  // Stessa disciplina: un fuso scritto male si vede all'avvio, non al primo render di una data
  // (piano 4.6.3, doc/icu.md).
  const icuOptions = normalizeIcuOptions(defs.icu);
  ...
  // Duale di leggiFlag: il default è spento, quindi accendono SOLO `true` esplicito o una
  ...
  const plugin = {
    ...
    // Stessa ragione, lato server: in SSR Vite esternalizza le dipendenze di node_modules, e
    // Node, importando "/react" da sé, muore su "virtual:" (ERR_UNSUPPORTED_ESM_URL_SCHEME).
    // `ssr.noExternal` lo fa passare dalla pipeline di Vite, dove il modulo virtuale esiste.
    // React e le altre dipendenze restano esterne: si include solo questo pacchetto.
    ...
    async config(_userConfig, env) {
      ...
      return {
        optimizeDeps: { exclude: ["@sepoina/vitetranslate"] },
        ssr: { noExternal: ["@sepoina/vitetranslate"] },
      };
    },
    ...
    resolveId(id) {
      ...
      if (id === VIRTUAL_ICU_ID) return RESOLVED_VIRTUAL_ICU_ID;
    },
    async load(id) {
