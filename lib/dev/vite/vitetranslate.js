...
export default function vitetranslate(defs) {
  ...
  function generateLanguagesModule() {
    ...
    const tags = files.map(f => f.replace(/\.json$/, ""));
    ...
    // defaultLanguage è SEMPRE inclusa: serve come fallback universale a runtime (in
    ...
    // opzionale `preloadedLanguages`, servono a evitare il flash quando il prop `predefined`
    ...
    const requested = [defs.defaultLanguage, ...(defs.preloadedLanguages ?? [])];
    const preloadedTags = [];
    for (const tag of requested) {
      if (preloadedTags.includes(tag)) continue; // dedup (default + eventuali duplicati)
      if (!tags.includes(tag)) {
        if (tag !== defs.defaultLanguage) {
    ...
    // per inizializzare il context in modo sincrono quando `predefined` è precaricata.
    const preloadedEntries = preloadedTags.map(tag => `  ${JSON.stringify(tag)}: ${binding[tag]}`);
    const defaultTableExpr = binding[defs.defaultLanguage] ?? "null";

    return (
      `${eagerImports.join("\n")}\n` +
      `export const languages = {\n${entries.join(",\n")}\n};\n` +
      `export const defaultLanguage = ${JSON.stringify(defs.defaultLanguage)};\n` +
      `export const defaultTable = ${defaultTableExpr};\n` +
