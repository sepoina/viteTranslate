    ...
    transform: {
      ...
      handler(code, id) {
        ...
        try {
          ...
          // @babel/core è una peer dependency opzionale: caricata pigramente alla prima
          // estrazione (vedi ensureBabel in extractMarkers.js), quindi il fallimento arriva
          // qui e non più al caricamento del modulo. Categoria a sé, perché senza Babel OGNI
          ...
          if (error?.code === "VT_NO_BABEL") {
            reporter.report("no-babel", `${error.message} It is an optional peer dependency of this plugin: \`npm i -D @babel/core\`.`);
            return null;
          }
    ...
    async configureServer(server) {
      ...
      if (!result.ok) {
        ...
        if (fixCommand) {
          ...
          // "$ " davanti, come un prompt di shell: la stessa convenzione di --fastverify in
          // cli.js, ed è quella che ogni man page e ogni README usano per dire "questo si
          // digita in un terminale, non è un'etichetta". Attenuato ("normale" = dim), perché
          // il prompt non è l'informazione: è il comando che deve saltare all'occhio.
          logEchoColored("", `${colorize("normale", "$")} ${colorize("ok", fixCommand)}`);
        } else {
