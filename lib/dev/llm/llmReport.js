...
//                 │
//                 │   real token: 2051 (≈ $0.0017)   [· reasoning 72% of output, se il modello ragiona]
 ...
 *
 * @param {{ connection?: { model?: string, baseURL?: string, modelClass?: { name: string } },
...
export function printEstimate({
  ...
  logEchoColored("LLM", `"${connection.model ?? "custom-driver"}" · ${connection.modelClass?.name ?? "standard"}`);
  logEchoColored(
    provider ? `⌘ ${provider}` : "",
 ...
 * Quando il modello ha ragionato, la sua quota dell'uscita: sono token pagati come la risposta,
 * e spesso ne sono la parte più grossa — è il primo numero da guardare prima di spegnerlo.
 *
 * @param {{ tokensIn?: number, tokensOut?: number, reasoningOut?: number, cost?: { cost: number },
 *   costUnity?: string, unknownKeys?: number }} params
 */
export function printRunResult({ tokensIn, tokensOut, reasoningOut = 0, cost, costUnity = "$", unknownKeys = 0 }) {
  const total = Math.round((tokensIn ?? 0) + (tokensOut ?? 0));
  if (total > 0) {
    const reasoning = reasoningOut > 0 && tokensOut > 0
      ? ` · reasoning ${Math.round((reasoningOut / tokensOut) * 100)}% of output`
      : "";
    logEchoColored("", "");
    logEchoColored("", `real token: ${total}${cost ? ` (≈ ${formatCost(cost.cost, costUnity)})` : ""}${reasoning}`);
  }
  if (unknownKeys > 0) {
