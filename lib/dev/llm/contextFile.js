...
const GENERATED_META_RE = /<!-- vitetranslate:generated (\S+) · (\d+) keys · (.+?) -->/;
...
/** Legge la riga di apertura dei marcatori: `{ date, keys, model }`, o `null` se assente —
 *  usato da `--llm-status` per raccontare il contesto senza duplicare la regex. */
export function readGeneratedMeta(text) {
  if (typeof text !== "string") return null;
  const match = text.match(GENERATED_META_RE);
  if (!match) return null;
  return { date: match[1], keys: Number(match[2]), model: match[3] };
}
 ...
 * rigenerano mai da soli (il primo solo su `--llm-context` esplicito, gestito dal chiamante).
 *
 * @param {{
