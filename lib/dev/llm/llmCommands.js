...
/** La riga `budget:` di `--llm-status`: il preset (o `custom`) e i tetti attivi, nell'unità in
 *  cui sono espressi — costo con i prezzi, token senza. */
function budgetLine(budget, priced, costUnity) {
  if (budget.preset === "unlimited") return "unlimited";

  const limit = (n, format) => (n === Infinity ? "no limit" : format(n));
  const money = (n) => limit(n, (v) => formatCost(v, costUnity));
  const tokens = (n) => limit(n, formatTokens);

  const parts = [];
  if (budget.maxCostPerRun !== undefined) parts.push(`${money(budget.maxCostPerRun)}/run`);
  if (budget.maxCostPerDay !== undefined) parts.push(`${money(budget.maxCostPerDay)}/day`);
  if (budget.maxTokensPerRun !== undefined) parts.push(`${tokens(budget.maxTokensPerRun)} tokens/run`);
  if (budget.maxTokensPerDay !== undefined) parts.push(`${tokens(budget.maxTokensPerDay)}/day`);

  const noPrices = budget.preset !== "custom" && !priced ? " (no prices)" : "";
  return `${budget.preset}${noPrices} — ${parts.join(", ")}`;
}
...
async function runLlmStatus(config, { ping, debug }) {
  ...
  const { costUnity, modelClass, maxTokensField } = llm.connection;
  const priced = llm.connection.costMillionInput != null;
  logEchoColored(
    "",
    `today: ${priced ? `${formatCost(ledger.costToday, costUnity)} · ` : ""}` +
    `${formatTokens(ledger.tokensToday)} tokens · ${ledger.keysToday} key(s)`
  );
  logEchoColored("", `budget: ${budgetLine(llm.budget, priced, costUnity)}`);
  logEchoColored(
    "",
    `model class: ${modelClass.name} — batches up to ${formatTokens(modelClass.maxOutputTokens)} output tokens / ` +
    `${modelClass.maxKeys} keys, ` +
    (maxTokensField ? `max_tokens ${modelClass.maxTokens} (${maxTokensField})` : "max_tokens not sent")
  );
  logEchoColored(
    "",
 ...
 *  tradotto niente. */
/** Le chiavi saltate perché già rifiutate due volte: restano `null` finché non si ritentano. */
function printSkipped(skipped) {
  if (!skipped?.length) return;
  const total = skipped.reduce((sum, l) => sum + l.keys, 0);
  logEchoColored("", `${total} key(s) skipped after two rejected attempts:`, "warning");
  for (const { tag, keys, reasons } of skipped) {
    const why = Object.entries(reasons).map(([reason, n]) => `${reason} ×${n}`).join(", ");
    logEchoColored("", `- ${tag}: ${keys} (${why})`, "warning");
  }
  logEchoColored("", "--llm-auto retries them; --llm-debug records the model's replies.");
}
...
function reportOutcome(result) {
  if (result.mode === "refused") {
    ...
    if (result.skipped?.length > 0) {
      logEchoColored("LLM", "nothing sent: the only keys left were rejected twice before.");
    } else {
      logEchoColored("LLM", "nothing to translate: every target language is complete.");
    }
    printSkipped(result.skipped);
  } else if (result.mode === "done") {
    printSkipped(result.skipped);
