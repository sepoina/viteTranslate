...
import { logEchoColored, colorize, displayWidth, clipToWidth, logTextWidth } from "../../utility.js";
import { nomeLingua } from "../vite/uty/syncReport.js";
import { costOf, formatCost, formatTokens } from "./costModel.js";
...
/** Una stringa larga almeno `w` colonne, con gli spazi a destra (`displayWidth`, non `length`). */
const allarga = (s, w) => `${s}${" ".repeat(Math.max(0, w - displayWidth(s)))}`;

/**
 * `destra` contro il bordo della colonna, `sinistra` al suo inizio: le colonne restano ferme
 * mentre le righe cambiano, invece di inseguire la riga più lunga del momento. Dal vivo
 * (`taglia`) una `sinistra` troppo lunga si tronca, perché una riga che si riscrive non può
 * andare a capo; nel log finale no — lì va a capo, e non si perde niente.
 */
function alDestra(sinistra, destra, larghezza, taglia) {
  const testo = taglia ? clipToWidth(sinistra, Math.max(1, larghezza - displayWidth(destra) - 2)) : sinistra;
 ...
 *   connection?: { costMillionInput?: number, costMillionOutput?: number, costUnity?: string },
 ...
 *   nei test: di norma la regione vive se e solo se stdout è un terminale.
 * @returns {{
...
export default function createRequestPanel({
  sourceTag, tags = [], connection = {}, traced = false,
  ...
  /** La riga di una richiesta, senza la parte destra. `tick === null`: nel log finale. */
  function sinistra(v, tick) {
    const soggetto = v.kind === "context" ? "context abstract" : lingua(v.tag);

    if (v.state === "run") {
      const indicatore = tick === null ? SOSPESO : SPINNER[tick % SPINNER.length];
    ...
    if (v.state === "error") {
      ...
      return `${colorize("error", FALLITO)} - error ${soggetto}${perche ? ` (${perche})` : ""}. ` +
        (traced ? "see the debug trace!" : "see trace in debug mode!");
    }

    if (v.kind === "context") {
      return `${colorize("ok", FATTO)} < context abstract, ${quanti(v.result.lines, "line")}`;
    }
    ...
    return `${colorize(completo ? "ok" : "warning", FATTO)} < ${arrivate} ${soggetto}.` +
      (completo ? " Full translate!" : "") + (resto.length ? ` ${resto.join(", ")}` : "");
  }
  ...
  /** Nel log finale, il costo di una richiesta: in valuta con i prezzi configurati, altrimenti
   *  i token misurati dal provider; niente se il provider (o il driver) non ne ha detti. */
  function costo(v) {
    if (v.state !== "done" || !v.usage) return "";
    const { tokensIn, tokensOut } = v.usage;
    const prezzo = costOf({ tokensIn, tokensOut, connection });
    return prezzo
      ? formatCost(prezzo.cost, connection.costUnity)
  ...
  function render({ tick, maxLines, width }) {
    ...
    const righe = mostrate.map((v) => {
      ...
      return alDestra(testo, destraDalVivo(v, width - displayWidth(testo) - 2), width, true);
    });
    if (nascoste.length) {
  ...
  function finish() {
    ...
    regione?.stop();
    regione = null;
    // Il costo in una colonna sua, i secondi in fondo come dal vivo: tutte le parti destre
    // sono larghe uguale, così i costi cominciano tutti nello stesso punto.
    const costi = voci.map(costo);
    const larghezzaCosti = Math.max(0, ...costi.map(displayWidth));
    const larghezzaSecondi = Math.max(0, ...voci.map((v) => secondi(v).length));
    voci.forEach((v, i) => {
      const tempo = colorize("normale", secondi(v).padStart(larghezzaSecondi));
      const destra = costi[i] ? `${allarga(costi[i], larghezzaCosti)}  ${tempo}` : tempo;
      logEchoColored(i === 0 ? "llm" : "", alDestra(sinistra(v, null), destra, logTextWidth(), false));
    });
