...
//   ⠹ > split 19 keys italiano - français                                  2s
...
export function shortReason(error) {
  if (error?.truncated) return "truncated";
 ...
 * @returns {{
 *   open(p: { kind?: "translate" | "split" | "repair" | "context", tag?: string, count: number }): {
 ...
 *   finish(): void,
 * }}
 *   `kind: "split"` è un lotto che rimanda le chiavi di una risposta troncata (translatePass.js).
 *   `done` riceve `{ filled, rejected, missing, unknown, remaining?, truncated? }` per una
 *   traduzione o una riparazione — `remaining`: le chiavi ancora da tradurre nella lingua dopo
 *   questo lotto; `truncated`: la risposta era tagliata, e se ne sono tenute le coppie complete —,
...
export default function createRequestPanel({
  ...
  function sinistra(v, tick) {
    ...
    if (v.state === "run") {
      ...
      const verbo = v.kind === "repair" || v.kind === "split" ? v.kind : "ask";
      const domanda = v.kind === "context"
        ? `ask context abstract from ${quanti(v.count, "string")}`
        : `${verbo} ${quanti(v.count, "key")} ${lingua(sourceTag)} - ${lingua(v.tag)}`;
    ...
    const { filled = 0, rejected = 0, missing = 0, unknown = 0, remaining, truncated = false } = v.result ?? {};
    const completo = filled === v.count;
    // "Full translate!" parla della lingua, non del lotto: con 450 chiavi ancora a `null` un lotto
    // riuscito dice quante ne restano. Senza `remaining` (chi non lo passa) si torna al lotto.
    const statoLingua = remaining === undefined
      ? (completo ? "Full translate!" : null)
      : (remaining === 0 ? "Full translate!" : `${remaining} to do`);
    const resto = [];
    if (truncated) resto.push("truncated");
    ...
    if (finale) {
      const dettagli = [...(remaining === undefined ? [] : [statoLingua]), ...resto];
      return `${base}${coda(dettagli.length ? dettagli.join(" / ") : "completed")}`;
    }
    // `✔ < 50 new keys français. 178 to do. 2 rejected` · `✔ < 28 new keys français. Full translate!`
    const stato = statoLingua ? ` ${statoLingua}${statoLingua.endsWith("!") || !resto.length ? "" : "."}` : "";
    return `${base}${stato}${resto.length ? ` ${resto.join(", ")}` : ""}`;
  }
