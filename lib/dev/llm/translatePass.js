...
import { estimateRun, costOf } from "./costModel.js";
...
/**
 * @param {{
 *   config: object, tags?: string[] | null, dryRun?: boolean, force?: boolean, yes?: boolean,
 *   contextOnly?: boolean, showContext?: boolean, retranslateTags?: string[] | null, now?: Date,
 * }} options
 */
export default async function translatePass(options) {
  ...
  if (showContext) {
    return { mode: "show-context", text: readContextFile(localeDir) };
  }

  const ciCheck = checkCI({ yes });
  if (!ciCheck.ok) return { mode: "refused", message: ciCheck.message };

  let apiKey;
  if (!llm.driver) {
    apiKey = (await resolveApiKey({ connection: llm.connection, baseDir: config.baseDir })).key;
  }
  ...
  if (contextOnly) {
    return { mode: "context", regenerated: contextRegenerated, text: contextText };
  }
  ...
  for (const tag of targetTags) {
    ...
    if (isRetranslate) {
      backupLanguageFile(filePath, languageFileName(tag), oldText, {
        kind: "erased", reason: "--retranslate requested",
      });
    }
  ...
  const allBatches = languageBatches.flatMap((lb) => lb.batches.map((batch) => ({ tag: lb.tag, batch })));
  ...
  const capsCheck = checkNumericCaps({
    ...
    keysToday: ledger.keysToday, budget: llm.budget, force, costUnity: llm.connection.costUnity,
  });
  if (!capsCheck.ok) return { mode: "refused", message: capsCheck.message, estimate, cost };

  printEstimate({
    tags: targetTags, keys: totalKeys, requests: totalRequests,
    tokensIn: estimate.tokensIn, tokensOut: estimate.tokensOut, cost, costUnity: llm.connection.costUnity,
  });

  if (dryRun) return { mode: "dry-run", estimate, cost, totalKeys, totalRequests };

  const confirm = await confirmProceed({ yes });
  if (!confirm.ok) return { mode: "refused", message: confirm.message };
  ...
  const runOneBatch = (tag, batch) => async () => {
    if (runState.stopped) return { tag, batch, translations: {}, usage: null, skipped: true };
    try {
      const result = await callModel({
        connection: llm.connection, driver: llm.driver, apiKey,
        systemPrompt: systemPromptFor(tag), userPayload: buildUserPayload(batch),
      });
      applyUsage(result.usage);
      if (result.usage) { measuredTokensIn += result.usage.tokensIn; measuredTokensOut += result.usage.tokensOut; }
      return { tag, batch, translations: result.translations, usage: result.usage };
    } catch (error) {
      return { tag, batch, translations: {}, usage: null, error };
    }
  };
  ...
  const applyResults = (results) => {
    for (const r of results) {
      ...
      for (const responseKey of Object.keys(r.translations ?? {})) {
        if (!batchKeys.has(responseKey)) outcomes.push({ tag: r.tag, key: responseKey, status: "unknown-key" });
      }

      for (const item of r.batch) {
        const candidate = r.translations?.[item.key];
        if (candidate === undefined) continue; // resta `null`: non è un errore
        ...
        if (validated.ok) {
          ...
          outcomes.push({ tag: r.tag, key: item.key, status: "rejected", reason: validated.reason });
        }
      }
    }
  };
  ...
  const firstPass = await runWithConcurrency(
    allBatches.map(({ tag, batch }) => runOneBatch(tag, batch)),
    llm.connection.maxConcurrency
  );
  applyResults(firstPass);
    ...
    if (rejectedByTag.size > 0) {
      ...
      for (const [tag, rejections] of rejectedByTag) {
        ...
        for (const batch of buildBatches(items)) {
          const reasons = Object.fromEntries(items.filter((i) => batch.some((b) => b.key === i.key)).map((i) => [i.key, i.reason]));
          repairTasks.push({ tag, batch, reasons });
        }
      }

      const repairResults = await runWithConcurrency(
        repairTasks.map(({ tag, batch, reasons }) => async () => {
          if (runState.stopped) return { tag, batch, translations: {}, usage: null, skipped: true };
          const userPayload = buildRepairUserPayload(batch, reasons);
          try {
            const result = await callModel({
              connection: llm.connection, driver: llm.driver, apiKey,
      ...
      // Sostituisce l'esito "rejected" precedente con quello del giro di riparazione, qualunque
      // sia — un solo giro, non due: non si ritenta più oltre questo punto.
      for (const r of repairResults) {
        ...
        for (const item of r.batch) {
          ...
          if (validated.ok) {
            ...
            outcomes[idx] = { tag: r.tag, key: item.key, status: "rejected", reason: validated.reason };
          }
        }
      }
    }
  ...
  printRunResult({
    perLanguage: perLanguageReport, tokensIn: measuredTokensIn, tokensOut: measuredTokensOut,
