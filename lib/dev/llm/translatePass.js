...
import { printEstimate, printRunResult, printCostGuard } from "./llmReport.js";
...
export default async function translatePass(options) {
  ...
  if (dryRun) {
    printCostGuard({ cost: cost?.cost, costGuard: llm.costGuard, costUnity: llm.connection.costUnity });
  ...
  const batchLabel = (tag, n) => `translate-${tag}-b${String(n).padStart(2, "0")}`;

  const runOneBatch = (tag, batch, n) => async () => {
  ...
  const firstPass = await runWithConcurrency(
    allBatches.map(({ tag, batch, n }) => runOneBatch(tag, batch, n)),
  ...
  if (!runState.stopped) {
    ...
    if (rejectedByTag.size > 0) {
      ...
      const repairResults = await runWithConcurrency(
        repairTasks.map(({ tag, batch, reasons, n }) => async () => {
          if (runState.stopped) return { tag, batch, translations: {}, usage: null, skipped: true };
          ...
          try {
            const result = await callModel({
              ...
              debug, label: `repair-${tag}-b${String(n).padStart(2, "0")}`,
            });
            applyUsage(result.usage);
            if (result.usage) { measuredTokensIn += result.usage.tokensIn; measuredTokensOut += result.usage.tokensOut; }
            return { tag, batch, translations: result.translations, usage: result.usage };
      ...
      for (const r of repairResults) {
        ...
        for (const item of r.batch) {
          ...
          if (validated.ok) {
            ...
            outcomes[idx] = { tag: r.tag, key: item.key, status: "rejected", reason: validated.reason, candidate };
          }
        }
      }
