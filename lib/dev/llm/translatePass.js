...
export default async function translatePass(options) {
  ...
  if (wantsRegeneration && !dryRun) {
    ...
    const panel = createRequestPanel({
      sourceTag: config.sourceLanguage, connection: llm.connection, traced: debug !== null,
    });
  ...
  const panel = createRequestPanel({
    sourceTag: config.sourceLanguage, tags: targetTags, connection: llm.connection, traced: debug !== null,
  });
  ...
  const applyResult = (r) => {
    ...
    for (const responseKey of Object.keys(r.translations ?? {})) {
      if (batchKeys.has(responseKey)) continue;
  ...
  const applyRepair = (r) => {
    ...
    const counts = {
      ...
      unknown: Object.keys(r.translations ?? {}).filter((key) => !batchKeys.has(key)).length,
    };
