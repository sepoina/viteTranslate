...
export default async function translatePass(options) {
  ...
  for (const tag of targetTags) {
    ...
    const keysToTranslate = wantedKeys.filter((key) => {
      if (auto) return true;
      if (failureCount(ledger, tag, key) >= 2) {
        skippedFailed++;
        const reason = ledger.failures?.[tag]?.[key]?.reason ?? "unknown";
    ...
    perLanguage.push({ tag, filePath, table, entries, skippedFailed, skippedReasons });
  }
  ...
  // Le chiavi fallite due volte restano null ma non partono: senza questo elenco "niente da
  // tradurre" sembrava "tutto tradotto" mentre --status contava ancora le stringhe mancanti.
  const skipped = perLanguage
    .filter((l) => l.skippedFailed > 0)
    .map((l) => ({ tag: l.tag, keys: l.skippedFailed, reasons: l.skippedReasons }));

  if (totalKeys === 0) {
    return { mode: "nothing-to-do", skipped };
  }
  ...
  const applyResult = (r) => {
    ...
    for (const item of r.batch) {
      ...
      if (validated.ok) {
        ...
        outcomes.push({ tag: r.tag, key: item.key, status: "rejected", reason: validated.reason, message: validated.detail?.message, candidate });
        counts.rejected++;
      }
    }
    return counts;
  ...
  const applyRepair = (r) => {
    ...
    for (const item of r.batch) {
      ...
      if (validated.ok) {
        ...
        if (idx !== -1) outcomes[idx] = { tag: r.tag, key: item.key, status: "rejected", reason: validated.reason, message: validated.detail?.message, candidate };
        counts.rejected++;
      }
    }
    return counts;
    ...
    if (debug) {
      for (const tag of new Set(perLanguage.map((l) => l.tag))) {
        const rejected = outcomes
          ...
          .map((o) => ({ key: o.key, source: sourceTable[o.key], candidate: o.candidate, reason: o.reason, message: o.message }));
      ...
      if (rejectedByTag.size > 0) {
        ...
        for (const [tag, rejections] of rejectedByTag) {
          const items = rejections.map((rej) => ({
            key: rej.key, text: llmSourceText(sourceTable[rej.key]), where: whereFromKey(rej.key),
            // Il codice da solo ("icu-args") non dice al modello cosa correggere: il messaggio sì.
            reason: rej.message ? `${rej.reason}: ${rej.message}` : rej.reason,
          }));
  ...
  return { mode: "done", perLanguage: perLanguageReport, filesWritten, stoppedOnBudget: Boolean(runState.stopped), skipped };
}
