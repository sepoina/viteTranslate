...
import { compiledMarker, SOURCE_OPEN, SOURCE_CLOSE, PLACEHOLDER } from "../../markerSyntax.js";
...
// Il genitore di un JSXText e' "host" (rende qualunque children gli si passi, stringa o
// elemento) se e solo se una di queste due forme. Vedi doc/ImplementationPlans/4_3_0.md,
// paragrafo "La regola del genitore host": deliberatamente conservativa, nel dubbio non e' host.
...
export default function extractMarkers(code, options) {
  ...
  // L'alias e' un identificatore che il transform inietta nel file dell'utente: se per caso
  // esiste gia', ombreggerebbe il suo. La ricerca e' per SOTTOSTRINGA e non per identificatore
  // vero, ed e' eccessiva apposta -- costa una scansione lineare su un file che stiamo gia'
  ...
  walk(ast.program, null, (node, parent) => {
    ...
    if (type === "TemplateElement") {
      ...
      if (autoWrap && isHostParent(parent)) {
        ...
        // e scarta quella che lo contiene. Lo splice del percorso non avvolto rimette in coda
        // gli a-capo e quindi perde lo spazio significativo (difetto preesistente, non toccato
        // qui); sul percorso avvolto lo si rimette come `{" "}`, che sopravvive a qualunque
        // genitore perche' e' un figlio a se', non testo scartabile.
        ...
        wrapped = true;
        // Un testo auto-avvolto non puo' ricevere argomenti: il tag e' autochiuso e non porta
        // un array di valori. Ogni %s finirebbe nel fallback letterale e a runtime renderebbe
        // il mark di argomento assente, che a schermo si scopre molto dopo.
