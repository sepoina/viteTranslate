...
export function registerMarker(inner, filename, table, baseDir, warn = defaultWarn) {
  const relPath = (baseDir ? pathCmd.relative(baseDir, filename) : filename || "unknown").replace(/\\/g, "/");
  ...
  if (inner.includes(OPEN)) {
    // `relPath` e non `filename`: è lo stesso percorso già calcolato per il checksum, relativo
    // alla radice del progetto — corto da leggere e, nel terminale di VS Code, cliccabile.
    warn(
      `nested markers in "${relPath}": "${inner}" was read as a single text. ` +
