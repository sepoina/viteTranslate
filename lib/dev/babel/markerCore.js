...
import { SOURCE_OPEN, SOURCE_CLOSE, MIN_SOURCE_MARKED } from "../../markerSyntax.js";
...
/**
 * La regola di whitespace che JSX applica a un testo, alla lettera: è la stessa di
 ...
 *
 * "Alla lettera" è verificato contro `preset-react`, non contro la memoria: una versione
 * scritta a intuito diverge in due punti — il tab interno e l'inizializzazione di `last` — ed
 * è il motivo per cui proprio quelle due righe portano un commento loro.
 *
 * Serve perché l'id è l'hash del testo: senza, l'indentazione del sorgente entra nella
 * chiave e riformattare il file la cambia — cioè orfana la traduzione, in silenzio. Con,
 * l'id dipende dal testo che React rende davvero, e niente altro.
 *
 * @param {string} raw - la fetta di sorgente coperta dal nodo `JSXText`, offset compresi
 * @returns {string}
 */
export function cleanJsxText(raw) {
  const lines = raw.split(/\r\n|\n|\r/);
  // `0` e non `-1`, come in Babel: in un testo di soli spazi senza a-capo non c'è nessuna riga
  ...
  for (let i = 0; i < lines.length; i++) {
    // I tab diventano spazi PRIMA di tutto il resto, e anche quelli in mezzo alla riga: è la
    // prima cosa che fa cleanJSXElementLiteralChild, ed è l'unico punto in cui una versione
    // scritta a intuito diverge. `<p>_%_a\tb_%_</p>` rende "a b" e deve hasharsi "a b".
    let line = lines[i].replace(/\t/g, " ");
    // Da qui in poi i tab non esistono più: si trimma sui soli spazi, come Babel.
...
/**
 * Estrae il testo su cui lavorare da un nodo, nella forma specifica del suo tipo e della sua
 * posizione.
 *
 * - `StringLiteral` fuori da un attributo -> `value` è già la stringa: gli escape JS sono veri
 *   escape e Babel non decodifica nessuna entità qui.
 * - `StringLiteral` figlio diretto di `JSXAttribute` (`t="_%_...&amp;..._%_"`) -> la fetta
 *   GREZZA del sorgente, virgolette escluse, coi soli fine riga normalizzati a `\n`. Non
 *   `node.value`: Babel decodifica le entità dentro una stringa-attributo JSX (non dentro un
 *   literal JS), e quella decodifica si sommerebbe a `decodeEntities` in fase di compilazione
 *   tabella — un `&lt;b&gt;` scritto apposta per mostrare il tag finirebbe per diventare un
 *   `<b>` vero. Letta grezza, l'entità arriva in tabella letterale e viene decodificata una
 *   volta sola, come in ogni altra posizione. I fine riga si normalizzano perché un attributo
 *   quotato è l'unica posizione in cui un CRLF sopravvive fino all'id — altrimenti lo stesso
 *   file, ripreso su Windows con `core.autocrlf`, produrrebbe una chiave diversa da quella
 *   prodotta su Linux. Il whitespace INTERNO (a-capo scritti a mano, indentazione) non si
 *   tocca: qui non è dello strumento, è dell'autore, e cambiarlo cambierebbe ciò che rende.
 * - `JSXText` -> la fetta grezza del nodo, passata da `cleanJsxText` (whitespace) e poi
 *   trimmata (gli spazi a cavallo che lo splice rimette in coda per conto suo). Stessa ragione
 *   della doppia decodifica di cui sopra: Babel decodifica le entità anche qui.
 ...
 *
 * @param {object} node
 * @param {string} code - il sorgente completo del file, per la fetta grezza di JSXText e
 *   attributo quotato. Nessun default: un chiamante che lo dimentica deve rompersi subito,
 *   non degradare in un marcatore che silenziosamente smette di essere riconosciuto.
 * @param {object} [parent] - il genitore del nodo, per distinguere un attributo quotato da un
 *   `StringLiteral` qualunque
...
export function markedTextOf(node, code, parent) {
  ...
  if (value === null || value.length < MIN_SOURCE_MARKED) return null;
  if (!(value.startsWith(SOURCE_OPEN) && value.endsWith(SOURCE_CLOSE))) return null;
  return value;
}

/** Il contenuto di un marcatore, senza i delimitatori. */
export const innerTextOf = (marked) => marked.slice(SOURCE_OPEN.length, -SOURCE_CLOSE.length);
