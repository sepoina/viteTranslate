...
// Stesso canale/default di extractMarkers.js: la console col prefisso del plugin quando
// nessuno dice altrimenti, il raccoglitore di devReporter.js quando il chiamante (il transform
// "vitetranslate:compile-locale") ne passa uno — vedi doc/structure.md § "Console output during dev".
  ...
  while ((match = TAG_RE.exec(input)) !== null) {
    ...
    if (closing) {
      ...
      if (depth > 0 && depth !== stack.length - 1) {
        warn(
          `mis-nested markup: </${tag}> closes across ` +
