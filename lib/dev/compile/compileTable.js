...
// di lingua resta autosufficiente (vedi ARG). Arriva da `errorSolve.noArrayChar`, quindi la
// stessa build lo scrive uguale in ogni chunk e uguale a quello che usa l'interpolazione a
// runtime — sono due strade diverse per la stessa regola, e devono dire la stessa cosa.
...
// prefisso `errorSolve.beginCharUntranslated`. Mappa a `1` e non array: a runtime è un lookup
 ...
 * `errorSolve.beginCharUntranslated`, quindi si emette solo quando quel prefisso è acceso —
...
export function compileLanguageModule(table, tag = "", sourceTable = null, options = {}) {
  const missingArg = options.missingArg ?? ERROR_SOLVE_DEFAULTS.noArrayChar;
