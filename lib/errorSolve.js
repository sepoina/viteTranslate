...
export function normalizeErrorSolve(given, warn = (m) => console.warn(m)) {
  ...
  if (givenMark !== undefined && givenMark !== null) {
    if (typeof givenMark !== "object" || Array.isArray(givenMark)) {
      warn(`[vitetranslate] errorSolve.mark must be an object, got ${Array.isArray(givenMark) ? "array" : typeof givenMark}: ignored, defaults used.`);
    } else {
      ...
      for (const key of MARK_KEYS) {
        ...
        if (typeof value !== "string") {
          warn(`[vitetranslate] errorSolve.mark.${key} must be a string (or false to disable), got ${typeof value}: default "${ERROR_SOLVE_DEFAULTS.mark[key]}" kept.`);
          continue;
        }
        out.mark[key] = value;
      }
...
function unknownKey(where, key, known, warn) {
  ...
  warn(
    `[vitetranslate] ${where}: unknown option "${key}", ignored.` +
