...
/**
 * Il k-esimo "%s" (contato da 0, fuori dalle citazioni ICU) diventa "{k}". Rispetta le
 * citazioni di MF1: "''" è un apostrofo letterale; un "'" seguito da "{ } # |" apre una
 * citazione, che si chiude al "'" successivo — dentro una citazione "%s" non si tocca.
 *
...
export function parseIcu(text, tag) {
  ...
  if (/\p{L}'\{/u.test(text)) {
    return { icu: true, ok: false, code: "icu-apostrophe", message: "an apostrophe right before \"{\" starts an ICU quote and hides the argument: write ’ (U+2019) or two apostrophes ''" };
  }

  const norm = normalizePlaceholders(text);
  if (norm.inBranch) {
