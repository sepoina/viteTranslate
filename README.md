...
| 📄 **Language files are auto-generated** | The tables in `localeDir` are created and updated by the sync command from the markers found in your source. |
...
| 🆕 **New languages auto-detected** | Drop a `.yml` file in the locale folder. No registration anywhere else. |
...
Each language is one `.yml` file in `localeDir`, named after its BCP 47 tag ([full list](doc/bcp47.md)) (`it-IT.yml`, `en-US.yml`, …), holding the translation table as a flat map. The **source language** file is fully autogenerated from the markers found in your source — you never hand-write it. Every **other language** starts as an empty file (`touch fr-FR.yml` is enough): the sync command populates it with the same keys set to `null`, and all you do is fill in the translations.

> **Coming from 3.x?** Language files used to be JS modules. Run `npx vitetranslate-prepare-translation-table --migrate` once to convert them (originals kept as `.bak-migrated-*`), then run the command again without the flag. See [Migrating from 3.x](#-migrating-from-3x).
...
Values are always double-quoted, one entry per line, no indentation. It is a **strict subset of YAML** — strict meaning that whatever this format accepts, a real YAML parser reads the same way, so your editor colours it and any YAML tool can read it. The subset is narrow on purpose: unquoted YAML scalars mangle exactly the text translators write (`%s è pronto` is a syntax error, `price 5 # off` silently truncates to `price 5`, `1.20` becomes the number `1.2`, `null` stops being the word "null"). Anything outside the subset is an error naming the **line number**, not a plausible wrong value.
...
No further registration needed: every `.yml` file found in `localeDir` becomes automatically available — `useTranslateLanguage()` lists it and `TranslateContainer` loads it lazily on request.
...
```bash
vitetranslate-prepare-translation-table --migrate
```

One-off conversion of 3.x language files (`<tag>.js`) to the 4.0 format (`<tag>.yml`) — see below. It only converts and exits; nothing else runs.
...
```bash
npx vitetranslate-prepare-translation-table --migrate   # <tag>.js  ->  <tag>.yml
npx vitetranslate-prepare-translation-table             # re-sync as usual
```
...
> [!WARNING]
> - **Ids are a 32-bit hash over the file's relative path and the text**, prefixed by the basename sanitised to `[A-Za-z0-9]` (`<name>_<hash>`). Two files sharing a basename in different folders no longer collide, because the path enters the hash. A 32-bit collision between two different strings is still unlikely but possible, and it is reported as a build warning naming both texts.
