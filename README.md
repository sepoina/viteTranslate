...
`<Translate>`, and the JSON translation tables are generated and kept in sync for you.
...
separator row so it's obvious what's left:
...
scans `srcDir` for `_%_..._%_` markers, and syncs every JSON file in `localeDir`: adds new
