...
**Plugin messages during `vite dev` follow their own rule: one warning per category, the rest on request.** A typo in a language file used to print a line on every save — the manifest regenerates each time a language file changes — and ten broken files printed ten lines at once. Now the first message in a given category (an invalid language file, an empty one, a malformed marker, and so on) prints in full; the rest of that category are just counted, closing with `+N more: run "npx vitetranslate --status" for the full list`. The block appears a quarter of a second after the last warning of the batch, so one page load is one block and not five. Reloading a page with the exact same problems as before prints nothing a second time — but only within the same `vite dev` process: restarting it always shows the full picture at least once, and if the count is unchanged from the previous run the closing line just says so ("same as the previous session").

During `vite dev` that batching is per **category**, without exception — every kind reported goes through the same "first message shown, the rest counted" rule above, `autoWrap`'s included. The extraction categories, in full: `nested` (two markers in one string), `collision` (two texts hashing to the same id), `malformed` (a `_%_` that doesn't open **and** close the whole string) and, since 4.4.0, three more from `autoWrap` — `marker-split` (a JSX tag broke a marker before extraction ever saw a whole one), `marker-newline` (a line break survives inside a marked attribute, changing both the rendered text and the id), `autowrap-noscope` (a marked text or attribute that `autoWrap` would rewrite, but no enclosing component could be recognised for the injection — see [plugin options](plugin-options.md)), and `autowrap-placeholder` (a `%s` inside a rewritten call, which has nowhere to take an argument). The sync command draws its own, separate line here: there, `nested` and `collision` are always listed in full — rare, and each about a different place in the code — while the other four are kept to a count unless run with `--status`, which lists everything.
